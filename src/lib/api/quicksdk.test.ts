...
describe('getQuickSdkGameFlow', () => {
  it('falls back to database flow rows when the aggregate endpoint fails', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ detail: 'aggregate failed' }), {
          status: 500,
    ...
    expect(String(fetchMock.mock.calls[1][0])).toContain('/api/quicksdk/flows')
  })
})
