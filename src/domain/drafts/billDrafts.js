...
function monthKey(value) {
  ...
  if (!raw) return ''
  const match = raw.match(/^(20\d{2})\D{0,3}(0?[1-9]|1[0-2])(?:\D|$)/)
...
export function normalizeChannelDraft(record = {}) {
  const settlementMonth = monthKey(record.settlementMonth)
  return {
    settlementMonth, channelName: text(record.channelName), partnerName: text(record.partnerName), invoiceStatus: text(record.invoiceStatus || record.invoice_status) || 'pending_invoice', remark: text(record.remark), status: text(record.status) || 'pending', serverCost: number(record.serverCost), discountType: text(record.discountType), channelFeeRate: number(record.channelFeeRate), devShareRate: number(record.devShareRate), profitRate: number(record.profitRate), settlementRuleCode: text(record.settlementRuleCode) || 'legacy_fixed_fee_tax', channelFeeMode: text(record.channelFeeMode) || 'fixed', taxMode: text(record.taxMode) || 'share', validationTolerance: number(record.validationTolerance, 0.05), validationStatus: text(record.validationStatus) || 'unvalidated', items: Array.isArray(record.items) ? record.items.map((line) => normalizeChannelLine(line, settlementMonth)) : []
  }
}
...
export function isMeaningfulChannelDraft(record = {}) {
  ...
  return normalized.items.some((line) => line.gameName || line.flow !== 0 || line.settlementCycle || line.voucherCost !== 0 || line.noWorryCost !== 0 || line.refundCost !== 0 || line.testCost !== 0 || line.welfareCost !== 0 || line.coinCost !== 0 || line.shareRate !== 30 || line.taxRate !== 5 || line.gatewayCost !== 0 || line.platformSettlementAmount != null || line.discountFactor !== 1 || line.settlementAmount !== 0)
}
