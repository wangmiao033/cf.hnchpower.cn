"""V3.0 final compatibility entrypoint with confirmation-blocking policy."""
...
app = FastAPI(
    title="contract-reconciliation-v7",
    docs_url=None,
    redoc_url=None,
    openapi_url=None,
)
...
def _apply_confirmation_policy(result: dict) -> dict:
    ...
    for source in result.get("lines") or []:
        ...
        if handling == "edit_bill" and case_status == "processing":
            # Merely choosing to edit cannot bypass the accounting gate. The
            # amount must actually change and be rechecked first.
            line["status"] = "fail"
