...
def test_past_channel_months_are_allowed_and_normalized() -> None:
    assert _line("2024-3").settlement_cycle == "2024-03"
    assert _line("2025年10月").settlement_cycle == "2025-10"


def test_current_channel_month_is_allowed() -> None:
    current = _current_month_key()
    assert _line(current).settlement_cycle == current


def test_future_channel_line_month_is_blocked() -> None:
    with pytest.raises(ValidationError, match="结算月份不能晚于当前月份"):
        _line(_next_month_key())


def test_future_channel_header_month_is_blocked_on_create_and_update() -> None:
...
def test_invalid_channel_month_format_is_rejected() -> None:
    with pytest.raises(ValidationError, match="结算月份必须使用 YYYY-MM 格式"):
        _line("2025-13")
